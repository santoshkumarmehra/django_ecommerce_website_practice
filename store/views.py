...
def filleddata(request):
    # input all data 
    prod_id = request.GET.get('prod_id')
    # print("start")
    # print(prod_id)
    filleddata_value = request.GET.get('filleddata_value')
    quantityvalue = int(filleddata_value)
    cart = Cart.objects.get(product=prod_id)
    cart.quantity = quantityvalue
    cart.save()
    user = request.user
    ...
    if cart_product:
        ...
        total_amount = amount 
    # print(total_amount)
    cart1 = Cart.objects.get(product=prod_id)
    # print("end")
    data = {
...
def success(request, id):
    if request.user.is_authenticated:
        ...
        cart = Cart.objects.filter(user=user)
        for item in cart:
            Order(user=user, product=item.product, quantity=item.quantity, transaction_id=id).save()
            item.delete()
        # print("success")
